...
(function () {
        ...
        if (pageSource && cnv && ctx) {
            ...
            if (osPageCtx) {
                ...
                Promise.all(imagePromises).then(() => {
                    pageSource.rules.forEach(rule => osPageCtx.fillRect(marginPixels + rule.x, marginPixels + rule.y, rule.w, rule.h));
    ...
    function drawBox(boxData) {
        const timeoutId = setTimeout(() => { if (renderComplete) {
            resumeDraw();
        } }, 100);
        function resumeDraw() {
...
//# sourceMappingURL=webviewFunctions.js.map